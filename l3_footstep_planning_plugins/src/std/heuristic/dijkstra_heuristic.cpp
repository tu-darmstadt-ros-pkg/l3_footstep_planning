...
bool DijkstraHeuristic::loadParams(const vigir_generic_params::ParameterSet& params)
{
  ...
  neighbors_dimension_ = std::sqrt(neighbor_costs_vector_.size());

  if (neighbors_dimension_ != std::floor(neighbors_dimension_) || static_cast<int>(neighbors_dimension_) % 2 == 0 || neighbors_dimension_ < 3)
  {
  ...
  for (int i = 0; i < neighbors_dimension_; i++)
  {
    ...
    {
      float cost = neighbor_costs_vector_[i * static_cast<int>(neighbors_dimension_) + j];
      row.push_back(cost);
      if (cost >= 0)
        num_neighbors_++;
    }
...
std::vector<l3::PositionIndex> DijkstraHeuristic::getNeighbors(const l3::PositionIndex& current_index) const
{
  ...
  neighbors.reserve(num_neighbors_);
